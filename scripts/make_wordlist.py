#!/usr/bin/env python3
"""Build a large brainwallet candidate list from public sources.

Downloads (and caches) common-password and dictionary lists, mixes in the
curated seeds under data/seeds.txt, emits a deduped quality prefix with
*light* mutations, then streams ranked two-word combinations so the default
run is well over 1 billion candidates. Full 2^n case explosion is
intentionally not used — that path is case_variants.py.

The combinatorial tail is generated by construction (no 1B-entry RAM set).
Pipe to brainflayer or write a ~20 GiB file. Sources are public wordlists,
not leaked full dumps.
"""
...
# Default cartesian sizes. 36000*(36000-1) space pairs is already >1e9;
# hyphen/concat/underscore on a smaller prefix add more without a 2^n blow-up.
DEFAULT_PAIR_SIZE = 36000
DEFAULT_HYPHEN_N = 15000
DEFAULT_CONCAT_N = 15000
DEFAULT_UNDER_N = 8000
DEFAULT_TARGET = 1_000_000_000
...
class Emitter:
    def __init__(self, handle, limit: int | None) -> None:
        self.handle = handle  # binary file object
        self.limit = limit
    ...
    def flush(self) -> None:
        if self._parts:
            self.handle.write(b"".join(self._parts))
            self._parts.clear()
            self._parts_n = 0

    def _push_line(self, line: bytes) -> None:
        self._parts.append(line)
        self._parts_n += 1
        if self._parts_n >= 16384:
            self.flush()
    ...
    def add(self, candidate: str) -> bool:
        ...
        self._push_line(candidate.encode("ascii", "ignore") + b"\n")
        self.count += 1
    ...
    def release_index(self) -> None:
        """Drop the uniqueness set before the combinatorial stream."""
        self.track = False
        self.seen.clear()

    def stream_ranked_pairs(
        self,
        words: Sequence[str],
        *,
        hyphen_n: int,
        ...
        for i in range(n):
            ...
            if i < concat_n:
                extra.append(empty)
            if i < under_n:
                extra.append(underscore)
            for j in range(i):
                right = encoded[j]
                self._push_line(left + space + right + nl)
                self._push_line(right + space + left + nl)
                added = 2
                for joiner in extra:
                    self._push_line(left + joiner + right + nl)
                    self._push_line(right + joiner + left + nl)
                    added += 2
                self.count += added
                if self.limit is not None and self.count >= self.limit:
                    self.flush()
            ...
            if i & 2047 == 2047:
                print(
                    f"    vocab {i + 1:,}/{n:,}  lines {self.count:,}",
                    file=sys.stderr,
                )
        self.flush()
        return True
...
def directed_pairs(n: int) -> int:
    return n * (n - 1) if n > 1 else 0


def combinatorial_count(n: int, hyphen_n: int, concat_n: int, under_n: int) -> int:
    cap = max(n, 0)

    def sized(k: int) -> int:
        return directed_pairs(min(max(k, 0), cap))

    return sized(n) + sized(hyphen_n) + sized(concat_n) + sized(under_n)


def pair_vocabulary(*groups: Sequence[str], size: int) -> list[str]:
    out: list[str] = []
    seen: set[str] = set()
    for group in groups:
        for word in group:
            ...
            if (
                item in seen
                or item in PAIR_STOP
                or not item.isalpha()
                or not (3 <= len(item) <= 12)
            ):
                continue
            seen.add(item)
            out.append(item)
...
def open_output(path: Path):
    ...
    return path.open("wb", buffering=8 * 1024 * 1024)
...
def build_wordlist(
    *,
    ...
    with open_output(output) as handle:
        ...
        if with_pairs and not core_only:
            vocab = pair_vocabulary(
                NOUNS,
                FIRST_NAMES,
                LAST_NAMES,
                google,
            ...
            hyphen = min(hyphen_n, len(vocab))
            concat = min(concat_n, len(vocab))
            under = min(under_n, len(vocab))
            estimated = combinatorial_count(len(vocab), hyphen, concat, under)
            print(
                f"[*] phase 5: streaming {estimated:,} ranked pairs "
                f"(vocab {len(vocab):,}; space={len(vocab):,}, "
                f"hyphen={hyphen:,}, concat={concat:,}, under={under:,})",
                file=sys.stderr,
            )
            if estimated + emitter.count < DEFAULT_TARGET:
                print(
                    f"[!] combinatorial tail {estimated:,} plus core "
                    f"{emitter.count:,} is under {DEFAULT_TARGET:,}; "
                    f"need a larger --pair-size or more dictionary sources",
                    file=sys.stderr,
                )
            before = emitter.count
            emitter.flush()
            emitter.release_index()
            if not emitter.stream_ranked_pairs(
                vocab, hyphen_n=hyphen, concat_n=concat, under_n=under
            ):
                emitter.flush()
                return emitter
            print(
                f"    {emitter.count - before:,} new (total {emitter.count:,})",
                file=sys.stderr,
            )
        elif with_pairs:
            print("[*] phase 5: two- and three-word phrases (core)", file=sys.stderr)
            before = emitter.count
...
def self_test() -> None:
    ...
    assert combinatorial_count(2, 0, 0, 0) == 2
    assert combinatorial_count(3, 0, 0, 0) == 6
    assert combinatorial_count(36000, 0, 0, 0) == 36000 * 35999
    assert combinatorial_count(36000, 15000, 15000, 8000) > DEFAULT_TARGET
    vocab = pair_vocabulary(["The", "bitcoin", "click", "dragon", "aa"], size=10)
    assert vocab == ["bitcoin", "dragon"]

    import io
    emitter = Emitter(io.BytesIO(), None)
    assert emitter.stream_ranked_pairs(["aa", "bb", "cc"], hyphen_n=0, concat_n=0, under_n=0)
    assert emitter.count == 6
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--max", type=int, default=0, help="stop after N phrases (0 = no cap)")
    ...
    parser.add_argument(
        "--core-only",
        action="store_true",
        help="quality prefix only (~3M); skip the 1B+ combinatorial tail",
    )
    parser.add_argument(
        "--pair-size",
        type=int,
        default=DEFAULT_PAIR_SIZE,
        help="dictionary words for ranked space-separated pairs (N*(N-1) lines)",
    )
    parser.add_argument("--hyphen-size", type=int, default=DEFAULT_HYPHEN_N)
    parser.add_argument("--concat-size", type=int, default=DEFAULT_CONCAT_N)
    parser.add_argument("--underscore-size", type=int, default=DEFAULT_UNDER_N)
    ...
    if args.pair_size < 2 and not args.core_only and not args.no_pairs:
        parser.error("--pair-size must be at least 2 for the combinatorial tail")
    ...
    emitter = build_wordlist(
        ...
        hyphen_n=args.hyphen_size,
        concat_n=args.concat_size,
        under_n=args.underscore_size,
    )
    print(
        f"[+] wrote {emitter.count:,} phrases to {args.output} "
        f"(skipped {emitter.skipped:,} empty/duplicate/invalid in the quality prefix)",
        file=sys.stderr,
    )
    if emitter.count < DEFAULT_TARGET and not args.core_only and not args.max:
        print(
            f"[!] output is under {DEFAULT_TARGET:,} candidates",
            file=sys.stderr,
        )
        return 1
    return 0
